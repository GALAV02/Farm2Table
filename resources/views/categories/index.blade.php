...
@section('content')
        ...
        <table class="w-full text-center border-collapse cursor-pointer">
            <thead>
                <tr class="bg-gray-100 text-gray-700">
                    <th class="border-b-2 p-4">C.id</th>
                    <th class="border-b-2 p-4">Category Name</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-lime-50 hover:shadow-md transition-all duration-300">
                    <td class="border-b p-4 font-medium text-gray-700">{{$category->cid}}</td>
                    <td class="border-b p-4 text-gray-600">{{$category->name}}</td>
                    <td class="border-b p-4 text-gray-600">
                        <a onclick="showEditPopup('{{$category->id}}', '{{$category->cid}}', '{{$category->name}}')" class="font-bold text-green-500 hover:text-green-600 hover:underline transition duration-200 mr-2">
                            Edit
                        </a>

                        <a onclick="showDeletePopup('{{$category->id}}')" class="font-bold text-red-500 hover:text-red-600 hover:underline transition duration-200">
                            Delete
                        </a>
                    </td>
                </tr>
                ...
                <form action="{{route('category.store')}}" method="POST" class="p-4 md:p-5">
                    ...
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <input type="number" name="cid" id="cid" value="{{$nextCid}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Enter category id" required>
                            @error('cid')
    ...
    <div id="popup-edit" class="fixed inset-0 bg-gray-600 bg-opacity-60 backdrop-blur-sm items-center justify-center hidden">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <form action="{{route('category.update')}}" method="POST" class="p-4 md:p-5">
                    @csrf
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2 sm:col-span-1">
                            <label for="cid" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Category ID :</label>
                            <input type="number" name="cid" id="edit_cid" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Enter category id">
                            @error('cid')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        ...
        function showEditPopup(id, cid, name) {
